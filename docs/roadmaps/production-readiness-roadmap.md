...
| --- | --- |
| Source intelligence | Remaining checker-owned Marrow facts for navigation surfaces and saved-data-backed rename application policy that still depend on BindingIndex ordering or editor-only refusal policy. Completion, signature help, semantic tokens, document/workspace symbols, module-path facts, source docs, operators, source hover, catalog-backed definition/references, and source-only rename facts are already Marrow-backed in the ledger. |
