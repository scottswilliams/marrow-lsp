...
| DAP | Store-generation facts, logpoint expression interpolation, virtual-source support, and serve/attach boundaries. Conditional breakpoints and stopped-frame watch/REPL/hover evaluation consume Marrow debug-expression facts, including durable-data expressions; exact hit-count breakpoints and static logpoints are DAP protocol behavior over Marrow stop-point facts. Local value expansion and the Durable Data scope consume Marrow runtime debug facts. |
