...
| DAP | Frame scopes, durable watch targets, store-generation facts, logpoint expression interpolation, and serve/attach boundaries. Conditional breakpoints and stopped-frame watch/REPL/hover local evaluation consume Marrow debug-expression facts; exact hit-count breakpoints and static logpoints are DAP protocol behavior over Marrow stop-point facts. Local value expansion already consumes Marrow runtime debug facts. |
