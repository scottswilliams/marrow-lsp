...
| --- | --- |
| Source intelligence | Remaining checker-owned Marrow facts for hover/navigation surfaces and saved-data-backed rename application policy that still depend on BindingIndex ordering or editor-only refusal policy. Completion, signature help, semantic tokens, document/workspace symbols, module-path facts, source docs, operators, many hover facts, catalog-backed definition/references, and source-only rename facts are already Marrow-backed in the ledger. |
| Data views | Read-only saved-root, child-page, and value-read inspection is a production data-view contract through Marrow typed saved-data DTOs, data-view boundaries, watch targets, and checked-program admission. Integrity and repair surfaces stay absent until Marrow exposes production validation facts, drift witnesses, and typed repair facts. Remaining work also includes debugger/runtime data-view contracts beyond read-only inspection, including served-process admission. |
