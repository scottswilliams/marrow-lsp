...
| MCP run | Stable public run-result DTOs, runtime generation facts, and serve/attach boundaries over the existing Marrow run/test sessions. Current run mode stays sandboxed over a fresh in-memory store without opening, reading, copying, or writing the configured project store, and now returns Marrow entry descriptor, analysis identity, footprint, cost-shape, and store-open-mode facts after entry admission. |
