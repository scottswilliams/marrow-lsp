...
| Hover: locals, params, functions, resources, enums, builtins, modules, roots | `presentation-only` | `type_at`, `BindingIndex`, `resolve_store_by_root` for saved-root declaration hover, `AnalysisSnapshot` catalog use-sites/declarations for enum type annotations, checked functions/resources/enums, direct effect facts, standard-library descriptors | Resource constructor and resource type-annotation leaf hovers consume `BindingIndex` resource references where Marrow emits them, including evolve transform body resource type refs. Enum type-annotation leaf hovers consume canonical catalog facts. The broader surface still needs checker-owned typed hover facts for declaration names, module paths, remaining type annotations, builtins, saved roots, enum member paths, docs, checked saved places, transitive effects, and versioned live-data binding before it can be a production semantic contract. Local token walking is presentation only and must shrink as Marrow facts appear. |
| Go-to definition, references, prepare rename, rename edits | `presentation-only` | `BindingIndex`, `SymbolRef`, `SymbolKind`, `RenameSafety`, `BindingIndex::rename_action`, `AnalysisSnapshot` catalog use-sites/declarations for saved roots, resource members, store indexes, enum type annotations, and enum members; parsed source for outline only | Saved roots, resource members, store indexes, enum type-annotation leaves, and enum member literals use canonical catalog spans. Imported module aliases, qualified function/resource/enum leaves, and evolve transform body resource type-annotation leaves consume BindingIndex facts where Marrow emits them; keyword-shaped alias prefixes and namespace-only prefixes stay unavailable instead of being tokenized locally. Source-only rename applies Marrow `SourceEdit` spans from `rename_action` when Marrow provides an action, rather than reconstructing edit ranges from source spelling. `prepareRename` uses the same Marrow rename-safety facts and refuses saved-data-backed or evolve-backed targets. Imported module aliases have no v0.1 alias syntax and are refused when Marrow returns no action. The broader surface remains presentation-only because complete module-prefix facts and durable identity facts for complete resource/store identity, aliases, and remaining non-covered type annotations are still absent. Source spelling is not durable identity. Saved-data-backed edits still refuse because no canonical editor application target exists for evolve intent. |
...
| Completion | `presentation-only` | `scope_at`, `resolve`, checked schemas/functions, syntax recovery, standard-library descriptors; saved-path declared-member dot completion consumes `marrow_check::tooling::declared_source_receiver_data_children` after recovering only the immediate receiver source before the cursor dot | Saved-path declared members and saved receiver key semantics are Marrow-owned facts. The broader completion surface still needs checker-owned completion context facts for namespace, expected type, active path, aliases, and recovery mode before it can be a production semantic contract. Do not infer durable context from nearby tokens beyond bounded cursor recovery. |
| Signature help | `presentation-only` | `marrow_check::tooling::active_callable_context`, checked function descriptors, `marrow_check::tooling::resource_constructor_signature`, and `marrow_check::tooling::intrinsic_callable_signature_for_file` | Source call shape, active argument, named argument, and suppression rules consume Marrow-owned active-call facts. Resource constructors, builtins, scalar conversions, `Error`, `Id`, and standard-library signatures consume Marrow typed facts with file-context import expansion. The surface still needs a checker-owned signature-help DTO that joins callable context, callable resolution, documentation, and parameter semantics before it can be a production semantic contract. |
| Custom LSP `marrow/savedRoots` | `presentation-only` | Typed saved-root views with Marrow `store_snapshot` metadata when `marrow.liveData` is enabled | Kept as the root listing entry point for the Saved Resource Inspector. TypeScript consumes the returned root segment instead of deriving child paths from display text, and it must not render key labels itself. Production use still needs versioned source/store generation, watch/refresh, serve, and attach contracts. |
...
| MCP `mw_saved_roots` | `presentation-only` | Typed root views with Marrow `store_snapshot` metadata when data access is enabled | Kept as a root listing helper. Production data serving still needs versioned source/store generation, integrity, repair, watch, serve, and attach contracts. |
...
| DAP conditional breakpoints | `presentation-only` | Non-empty `condition` expressions on verified Marrow stop-point lines are checked by `AnalysisSnapshot::checked_debug_expression` at that stop and must type as `bool`; armed conditions are evaluated on the parked run thread with `Frame::evaluate_debug_expression`, and the stop is taken only for `true`. Invalid or non-bool conditions return an `invalid-params` breakpoint contract and are not armed. | Virtual sources, serve/attach boundaries, and source/store generation remain blocked. |
| DAP exact hit-count and static logpoint breakpoints | `presentation-only` | Exact positive integer `hitCondition` values are DAP hit-count bookkeeping over verified Marrow stop-point lines. Static `logMessage` values emit console output and continue without stopping. Non-string or malformed hit/logpoint fields return `invalid-params` contracts and are not armed. | Logpoint expression interpolation, virtual sources, serve/attach boundaries, and source/store generation remain blocked. |
...
| DAP watch/REPL/hover local evaluate | `presentation-only` | While stopped, `watch`, `repl`, `hover`, and absent-context expressions are checked against the stopped frame file/span by `AnalysisSnapshot::checked_debug_expression`, evaluated on the parked run thread with `Frame::evaluate_debug_expression`, and returned with the same `DebugValue` presentation and expandable-child contract as locals. Invalid expressions fail with `invalid-params`; runtime rejections fail with `runtime-error`; not-stopped requests fail with `invalid-state`. | Durable watch/path expressions, virtual sources, serve/attach boundaries, and source/store generation remain blocked. |
| DAP durable variables/watch/path | `blocked-on-marrow` | Durable variable references and `^` watch/path expressions keep the stable durable-data blocked contract. Malformed evaluate envelopes fail with `invalid-params`; no durable path parser or production value explorer is present. | DAP still needs canonical durable watch/path facts, virtual sources, and source/store generation facts before those surfaces can graduate. |
| DAP disconnect/terminate/output | `ready` | Adapter lifecycle and captured runtime output. `terminate` succeeds for parked runs and staged resumes that have not physically woken the run thread; already-active runs reject `terminate` with `invalid-state`, and `disconnect` remains the shutdown path. | No language semantics. |
...
| VS Code debug contribution and F5 snippets | `presentation-only` | Launch schema for project, explicit entry selectors, typed args, exact string forms for int/date/bytes, stop-on-entry/stepping/plain line breakpoint UI, and DAP-advertised conditional, exact hit-count, static logpoint, and stopped-frame hover evaluation support | TypeScript only contributes schema/snippets and launches `marrow-dap`; it does not parse or validate Marrow entry values or breakpoint expressions. Durable-data debugger inspection and logpoint expression interpolation remain blocked. |
...
- VS Code launch snippets that seed invalid placeholder entries or raw positional
  args.
- DAP logpoint expression interpolation and durable watch parsing.
