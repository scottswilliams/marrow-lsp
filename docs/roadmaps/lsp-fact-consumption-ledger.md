...
| DAP initialize and shutdown transport | `ready` | DAP protocol handling and static capability response | Advertised capabilities must stay limited to implemented rows. Hover evaluate is not advertised. The DAP `initialized` event is emitted after launch preparation, when Marrow stop-point facts are available for configuration requests. |
...
| DAP launch with configured `defaultEntry` | `presentation-only` | `marrow_run::ProjectSession` opened with isolated writes, canonical entry descriptor invocation, typed argument DTOs, runtime stop-point facts, and the Marrow debugger hook | Configured-entry launch does not create accepted catalog state or native stores for absent data dirs, and writes stay isolated. Malformed launch envelope fields fail with `invalid-params`; optional `entry` and `stopOnEntry` `null` values remain absent/default. Existing native stores that would require catalog artifact repair are blocked until Marrow exposes read-only debug catalog admission facts. Production debug launch still needs frame-scope, value-expansion, store-generation, and serve/attach facts before graduating beyond a narrowed debug helper. |
| DAP explicit `entry` launch argument | `presentation-only` | Explicit string selectors are handed to `ProjectOpen::with_entry_override`, then admitted by Marrow's current canonical entry descriptor before launch succeeds; concrete non-string values fail with `invalid-params` | The selector is a transport convenience, not durable identity. Production debug launch still inherits the DAP frame/value, virtual-source, store-generation, and serve/attach blockers. |
| DAP non-empty launch `args` | `presentation-only` | Launch args decode to Marrow's typed entry DTOs and are admitted by `CheckedEntryCall::from_protocol_invocation`; non-array values, including `null`, fail with `invalid-params` | The adapter owns only JSON envelope validation. Marrow owns parameter names, catalog IDs, typed values, and admission against the checked program. Production debug launch still inherits the DAP frame/value, virtual-source, store-generation, and serve/attach blockers. |
| DAP `setBreakpoints` plain line arming | `presentation-only` | Plain line breakpoints are checked against `CheckedRuntimeProgram::stop_points()` plus runtime file ids/paths. Verified stop-point lines are armed before the run thread starts and can be updated while the run is parked; lines outside Marrow stop points return an `invalid-params` breakpoint contract. Out-of-sequence pre-launch requests remain unverified and unarmed. Malformed source envelopes and non-array breakpoint lists fail with `invalid-params`; positive `sourceReference` sources fail with `unsupported-request` because the adapter exposes no virtual-source surface. | Conditional breakpoints, virtual sources, serve/attach boundaries, and full frame/value semantics are still outside this narrowed debug helper. |
| DAP conditional, hit-condition, and logpoint breakpoints | `blocked-on-marrow` | Inputs receive a distinct blocked contract and are not evaluated, echoed, or armed. | Needs canonical breakpoint-expression facts and frame-scope evaluation facts. |
| DAP stop-on-entry and stepping | `presentation-only` | Statement spans and source file paths from the checked runtime hook. Malformed resume `threadId` envelopes fail with `invalid-params`. | Plain line breakpoints now consume Marrow stop-point facts; production debugger semantics still need frame-scope, value, serve/attach, and store-generation facts. |
| DAP threads, stack trace, scopes, locals, expandable locals | `presentation-only` | Single-thread debug adapter state, checked runtime source paths, and public runtime values where available. Malformed stack `threadId`, scope `frameId`, `variablesReference`, variable paging, and variable filter envelopes fail with `invalid-params`; scopes require a current stopped frame; unknown positive variable references remain empty for stale-reference tolerance. | Runtime value expansion, frame-scope, and virtual-source facts are still missing; values carry blocked contract metadata rather than production type claims. |
...
| VS Code debug contribution and F5 snippets | `presentation-only` | Launch schema for project, explicit entry selectors, typed args, exact string forms for int/date/bytes, stop-on-entry/stepping/plain line breakpoint UI | TypeScript only contributes schema/snippets and launches `marrow-dap`; it does not parse or validate Marrow entry values. Conditional, hit-condition, and logpoint breakpoints remain blocked. |
...
| Debugger facts | Frame scopes, production expression evaluation, value expansion, durable watch targets, store-generation facts, typed path segments, and serve/attach boundaries | DAP |
...
- VS Code launch snippets that seed invalid placeholder entries or raw positional
  args.
- DAP conditional/logpoint breakpoint expressions and durable watch parsing.
