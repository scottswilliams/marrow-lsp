...
| DAP threads, stack trace, scopes, locals, expandable locals | `presentation-only` | Single-thread debug adapter state, checked runtime source paths, and `Frame::debug_snapshot`/`DebugValue` for bounded local values and expandable local children. Malformed stack `threadId`, scope `frameId`, `variablesReference`, variable paging, and variable filter envelopes fail with `invalid-params`; scopes require a current stopped frame; unknown positive variable references remain empty for stale-reference tolerance. | Local variables no longer carry blocked runtime-value metadata, but threads, stack trace, scopes, virtual sources, source/store generation, and frame-scope evaluation remain outside a production debugger contract. |
| DAP durable variables/watch/evaluate/hover evaluate | `blocked-on-marrow` | Stable failed responses; malformed evaluate envelopes fail with `invalid-params`; no durable path parser, frame-scope evaluator, or production value explorer | Marrow exposes read-only expression evaluation and saved-data path candidates, but DAP still needs canonical frame-scope, durable watch/path, stop source, and source/store generation facts before production debugger evaluation. |
