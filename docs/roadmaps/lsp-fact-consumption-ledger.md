...
| --- | --- | --- | --- |
| DAP launch entry invocation | Configured and explicit-entry launch uses isolated Marrow project sessions and remains a narrowed debug helper | `SessionEntry`, canonical entry DTOs, typed launch-argument DTOs, debugger facts | Entry selectors and typed args are admitted through Marrow DTOs. Production graduation still needs debugger stop/source/frame/value facts and serve/attach boundaries. |
...
| DAP launch with configured `defaultEntry` | `presentation-only` | `marrow_run::ProjectSession` opened with isolated writes, canonical entry descriptor invocation, typed argument DTOs, and the Marrow debugger hook | Configured-entry launch does not create accepted catalog state or native stores for absent data dirs, and writes stay isolated. Malformed launch envelope fields fail with `invalid-params`; optional `entry` and `stopOnEntry` `null` values remain absent/default. Existing native stores that would require catalog artifact repair are blocked until Marrow exposes read-only debug catalog admission facts. Production debug launch still needs canonical stop-point, source mapping, frame-scope, value-expansion, and store-generation facts before graduating beyond a narrowed debug helper. |
| DAP explicit `entry` launch argument | `presentation-only` | Explicit string selectors are handed to `ProjectOpen::with_entry_override`, then admitted by Marrow's current canonical entry descriptor before launch succeeds; concrete non-string values fail with `invalid-params` | The selector is a transport convenience, not durable identity. Production debug launch still inherits the DAP stop/source/frame/value and store-generation blockers. |
| DAP non-empty launch `args` | `presentation-only` | Launch args decode to Marrow's typed entry DTOs and are admitted by `CheckedEntryCall::from_protocol_invocation`; non-array values, including `null`, fail with `invalid-params` | The adapter owns only JSON envelope validation. Marrow owns parameter names, catalog IDs, typed values, and admission against the checked program. Production debug launch still inherits the DAP stop/source/frame/value and store-generation blockers. |
...
| VS Code debug contribution and F5 snippets | `presentation-only` | Launch schema for project, explicit entry selectors, typed args, exact string forms for int/date/bytes, stop-on-entry/stepping/advisory breakpoint UI | TypeScript only contributes schema/snippets and launches `marrow-dap`; it does not parse or validate Marrow entry values. Line breakpoint arming remains blocked. |
...
| Debugger facts | Stop points, source mapping, frame scopes, production expression evaluation, value expansion, durable watch targets, store-generation facts, and typed path segments | DAP |
