...
| MCP surface read operations | Stable Marrow surface ABI, route manifest, operation request/response/error DTOs, and read-only project surface session | `marrow_json::surface::{SurfaceAbiJson, SurfaceRouteManifestJson, SurfaceOperationRequestJson, execute_project_surface_operation_read_only}` and `marrow_run::ProjectSurfaceReadSession` | `mw_surface_routes` exposes read routes without store access. `mw_surface_read` is gated by MCP data access and delegates read execution and write/action rejection to Marrow's canonical read-only executor. Raw `mw_data_*` tools remain presentation-only. |
...
| MCP `mw_surface_routes` | `ready` | `SurfaceAbiJson::from_program` and `SurfaceRouteManifestJson::from_abi`, filtered to read operations | Returns the Marrow-owned surface route manifest for read operations only. It does not open the data store and does not require the MCP data-access opt-in. |
