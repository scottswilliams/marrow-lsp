...
| DAP threads, stack trace, scopes, locals, expandable locals | `presentation-only` | Single-thread debug adapter state and public runtime values where available. Malformed `variablesReference` envelopes fail with `invalid-params`; unknown positive references remain empty for stale-reference tolerance. | Runtime value expansion facts and stable source mapping are still missing; values carry blocked contract metadata rather than production type claims. |
