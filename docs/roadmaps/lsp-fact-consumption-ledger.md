...
| --- | --- | --- | --- |
| MCP `mw_run` entry and args | Presentation helper blocks non-empty args and unstable entry identity | `ProjectSession`, `ProjectOpen`, `SessionEntry`, `CheckedEntryCall` | Replace manual run setup with session-backed invocation and typed result DTOs. |
...
| MCP `mw_run` pure execution/test helper | `presentation-only` | Checked zero-argument entry-call API over a fresh in-memory `TreeStore` and locked host | Runs only after normal check succeeds and no accepted-catalog baseline is pending. Current code still blocks non-empty args and treats explicit `entry` as presentation. Marrow session APIs must be evaluated and adopted before graduating run/debug behavior. |
| MCP `mw_run` durable catalog admission | `presentation-only` | Stable unavailable envelope; no catalog or store write from the current MCP helper | Marrow session and store APIs now provide production admission candidates. Current MCP behavior remains narrowed until it adopts those APIs and proves store/catalog generation boundaries. |
