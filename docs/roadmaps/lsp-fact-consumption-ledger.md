...
| Signature help | `presentation-only` | Checked function descriptors, `marrow_check::tooling::resource_constructor_signature`, and source call shape | Resource constructor signatures consume the Marrow typed fact. Needs checked active-call facts, argument position, suppression rules, builtin signatures, and broader callable facts before it can be a production semantic contract. |
