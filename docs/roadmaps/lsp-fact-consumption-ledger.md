...
| Saved Resource Inspector children and values | Typed child expansion and bounded value previews behind `marrow.liveData` | Shared saved-data path resolution, child, read, walk, render, and cursor APIs | Root listing, child pages, and value reads carry Marrow `store_snapshot` metadata. Value reads need bounded or streaming primitives before live data is production-stable. |
| MCP data children and reads | Typed saved-data path protocol through the shared core DTOs | Shared saved-data path resolution, read, child, walk, render, and cursor APIs | Child pages carry generation metadata. Stable production DTOs remain before graduating beyond presentation-only. |
...
| Custom LSP `marrow/dataChildren` and `marrow/dataRead` | `presentation-only` | Typed saved-data path segments, bounded child pages, Marrow-rendered labels, and bounded value previews over the committed native dev store; child pages and value reads carry Marrow `store_snapshot` metadata | Value reads need bounded or streaming primitives plus stable production DTOs before live saved-data inspection can be a production semantic contract. |
...
| MCP `mw_data_children` | `presentation-only` | Typed saved-data path segments, bounded child pages, and Marrow `store_snapshot` metadata over the real store when data access is enabled | Needs stable production DTOs before graduating. |
...
| VS Code Saved Resource Inspector and refresh command | `presentation-only` | Custom LSP saved-root, child-page, and value-read requests | View expands committed native-store resources when `marrow.liveData` is enabled. Root, child-page, and value replies carry Marrow `store_snapshot` metadata; the view does not watch uncommitted writes or debuggee state. |
