...
| Signature help | `presentation-only` | Checked function descriptors, `marrow_check::tooling::resource_constructor_signature`, `marrow_check::tooling::intrinsic_callable_signature_for_file`, and source call shape | Resource constructors, builtins, scalar conversions, `Error`, `Id`, and standard-library signatures consume Marrow typed facts with file-context import expansion. Needs checked active-call facts, argument position, suppression rules, and broader callable facts before it can be a production semantic contract. |
