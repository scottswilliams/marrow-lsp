...
| DAP `setBreakpoints` line arming | `blocked-on-marrow` | Advisory unverified breakpoint response; no line is armed. Conditional, hit-condition, and logpoint inputs receive a distinct blocked contract and are not evaluated or echoed. | Needs canonical source mapping, stop-point, frame-scope, and breakpoint-expression facts. |
