...
pub fn tools() -> Json {
    json!([
        ...
        {
            "name": "mw_complete",
            "description": "Presentation-only development helper: list Marrow source completion items (in-scope names, resource fields, saved roots, std ops, keywords) at a position in `file`. Missing remaining expected enum value contexts such as assignments and match arms, remaining checker-owned candidate facts, and a production stability contract; not a stable production completion API.",
