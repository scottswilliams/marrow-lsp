...
pub fn tools() -> Json {
    json!([
        ...
        {
            "name": "mw_complete",
            "description": "Presentation-only development helper: list Marrow source completion items (in-scope names, resource fields, saved roots, std ops, keywords) at a position in `file`. Missing expected-type context, active saved-path context, remaining checker-owned candidate facts, and a production stability contract; not a stable production completion API.",
