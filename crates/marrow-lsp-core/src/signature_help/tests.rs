...
#[test]
fn signature_help_has_no_local_callable_resolution_or_doc_walk() {
    let sources = [
        include_str!("../signature_help.rs"),
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "signature help LSP must consume Marrow signature-help facts instead of local {forbidden}"
        );
    }

    let local_model =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/signature_help/signatures.rs");
    assert!(
        !local_model.exists(),
        "signature help must not keep a local callable resolver at {}",
        local_model.display()
    );
}
